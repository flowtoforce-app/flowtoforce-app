...
export default function SeanceV2({ seance, seanceId, chapitre, token }) {
  ...
  return (
    <>
      <Head>
        <title>{`${seance.titre} — FlowToForce V2`}</title>
        <meta name="robots" content="noindex" />
      </Head>
      <div className={styles.page}>
        <header className={styles.header}>
          <Link href={`/programme/v2?token=${token}`} className={styles.back}>←</Link>
        ...
        <div className={styles.seanceActions}>
          ...
          <Link href={`/programme/v2?token=${token}`} className={styles.btnRetour}>
            Retour au programme
          </Link>
        </div>
...
export async function getServerSideProps({ params, query }) {
  ...
  const chapitre = chapitresV2.find(ch => ch.seances.includes(seanceId)) || null
